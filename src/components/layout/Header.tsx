...
import { useActiveDocument, useDocumentActions, useUpdateActiveContent, useTabs } from '@/store/documentStore';
import { useTheme } from '@/store/settingsStore';
import { usePanelLayout } from '@/store/editorStore';
...
export function Header() {
  ...
  useEffect(() => {
    if (!urlModalOpen && !csvModalOpen && !splitDocModalOpen) {
      return;
    }
  ...
  }, [urlModalOpen, csvModalOpen, splitDocModalOpen]);
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-bg-elevated rounded-lg shadow-xl border border-border-default max-h-[80vh] flex flex-col">
            <div className="px-4 py-3 border-b border-border-default">
              <h2 className="text-sm font-medium text-text-primary">Import CSV: {csvPreview.name}</h2>
                    ...
                    {csvPreview.rows.slice(0, 10).map((row, i) => (
                      <tr key={i} className="border-b border-border-subtle last:border-0">
                        {csvPreview.headers.map((header, j) => (
                          <td key={j} className="px-2 py-1.5 text-text-primary whitespace-nowrap max-w-[200px] truncate">
                            {String(row[header] ?? '')}
                          </td>
                        ))}
                      </tr>
            ...
            <div className="flex justify-end gap-2 px-4 py-3 border-t border-border-default bg-bg-surface/50 rounded-b-lg">
      ...
      {urlModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-bg-elevated rounded-lg shadow-xl border border-border-default">
            <div className="px-4 py-3 border-b border-border-default">
            ...
            <div className="p-4">
              <input
                ...
                className="w-full px-3 py-2 text-sm bg-bg-base border border-border-default rounded text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent"
                onKeyDown={(e) => {
      ...
      {splitDocModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-bg-elevated rounded-lg shadow-xl border border-border-default">
            <div className="px-4 py-3 border-b border-border-default">
                ...
                tabs
                  ...
                  .map(tab => (
                    <button
                      ...
                      className="w-full px-3 py-2 text-sm text-left text-text-secondary hover:text-text-primary hover:bg-bg-hover rounded flex items-center gap-2"
                    >
