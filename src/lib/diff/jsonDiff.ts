...
export function formatDiffEntry(entry: DiffEntry): string {
  switch (entry.type) {
    case 'added':
      return `+ ${entry.path}: ${JSON.stringify(entry.newValue)}`;
    case 'removed':
      return `- ${entry.path}: ${JSON.stringify(entry.oldValue)}`;
    case 'changed':
      return `~ ${entry.path}: ${JSON.stringify(entry.oldValue)} → ${JSON.stringify(entry.newValue)}`;
    default:
      return `  ${entry.path}`;
  }
}
 ...
 * Get summary of diff
 */
export function getDiffSummary(diffs: DiffEntry[]): {
  added: number;
  removed: number;
  changed: number;
  total: number;
